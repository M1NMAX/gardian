...
const Customs = () => {
  ...
  const { data, refetch } = useQuery<CustomItemInterface[]>(
    'customItems',
    async (): Promise<CustomItemInterface[]> => {
      const res = await fetch(
        process.env.NEXT_PUBLIC_API_URL +
          '/customItems/collectionId/' +
          collectionId
      );
  ...
  const { data: collection } = useQuery<CollectionInterface>(
    'customCollection',
    ...
    <div className='flex justify-between items-center px-2 py-1 shadow-md rounded bg-gray-100 dark:bg-gray-800 '>
      ...
      <Menu as='div' className='relative'>
        ...
        <Transition
          ...
          <Menu.Items
            ...
            <Menu.Item as='li'>
              <button
                onClick={renameItemModal.openModal}
            ...
            <Menu.Item>
              <button
                onClick={deleteItemModal.openModal}
                className='w-full space-x-1 btn btn-secondary'>
      ...
      {editItemModal.isOpen && (
        <EditCustomItemModal
          ...
          handleClose={editItemModal.closeModal}
          positiveFeedback={positiveFeedback}
          negativeFeedback={negativeFeedback}
        />
      )}

      {renameItemModal.isOpen && (
        <RenameModal
