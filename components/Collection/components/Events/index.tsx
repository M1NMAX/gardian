...
const Events = () => {
  ...
  const { data, refetch } = useQuery<EventInterface[]>(
    'events',
    async (): Promise<EventInterface[]> => {
      const res = await fetch(
        process.env.NEXT_PUBLIC_API_URL + '/events/collectionId/' + collectionId
      );
...
const Event: FC<EventProps> = ({ event }) => {
  ...
  return (
    <div className='relative p-1 rounded shadow-md bg-gray-100 dark:bg-gray-800 group'>
      <button
        onClick={editEventModal.openModal}
        className='grow flex flex-col space-y-1'>
        <span className='text-left font-medium truncate'>{event.name}</span>
      ...
      <div className='absolute top-0 right-0 z-10 md:invisible md:group-hover:visible'>
        <Menu as='div' className='relative '>
          ...
          <Transition
            ...
            <Menu.Items
              as='ul'
              className='absolute  z-10 -right-2 w-fit p-1 rounded border  origin-top-right bg-white dark:bg-gray-900'>
              <Menu.Item>
                <button
                  onClick={renameModal.openModal}
